...
const SpotifySearch = ({ searchCategory }) => {
  ...
  const [searchingDomain, setSearchDomain] = useState('song');
  const {token, songs, loading} = useSelector((state) => state.spotify);
    ...
    return (
      <>
        ...
        <h1>Search for {searchingDomain}</h1>

        <ul className="list-group">
          <li className="list-group-item">
            <form id = {"searchForm"} onSubmit={e => e.preventDefault()}>
            Search By
            <select id={"searchRule"} defaultValue={searchingDomain} onChange={(e) => {
                ...
                </li>
                {
                    songs && songs.map((song) =>
                        <li key={song.id} className="list-group-item">
                          ...
                          <Link to={`/${searchingDomain}/${song.id}`} state={{song: song}}>
                            Detail
                          </Link>
